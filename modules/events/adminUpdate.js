        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                 if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage({body:`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật\nhttps://www.facebook.com/profile.php?id=${logMessageData.TARGET_ID} thành qtv nhóm`, attachment: (await global.nodemodule["axios"]({
...
responseType: "stream"
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage({body:`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật\nhttps://www.facebook.com/profile.php?id=${logMessageData.TARGET_ID} Trở Về Thành Viên`, attachment: (await global.nodemodule["axios"]({
...
responseType: "stream"
            ...
            case "log:user-nickname": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage({body:`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật biệt danh của ${logMessageData.participant_id} Thành: ${(logMessageData.nickname.length == 0) ? "Tên Cũ": logMessageData.nickname}`, attachment: (await global.nodemodule["axios"]({
url: (await global.nodemodule["axios"]('https://API-VD.miraiprofile2005.repl.co/images/videoanime')).data.data,
method: "GET",
responseType: "stream"
})).data                                      }, threadID, async (error, info) => {
            ...
            case "log:thread-name": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage({body:`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật tên box thành ${dataThread.threadName}`, attachment: (await global.nodemodule["axios"]({
url: (await global.nodemodule["axios"]('https://API-VD.miraiprofile2005.repl.co/images/videoanime')).data.data,
method: "GET",
responseType: "stream"
})).data                                      }, threadID, async (error, info) => {
            ...
            case "log:thread-icon": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage({body:`» [ CẬP NHẬT NHÓM ]\n» Đã cập nhật icon mới ${event.logMessageBody.replace(" thành", "icon")}\n» icon cũ..: ${preIcon[threadID] || "Chịu =))"}`, attachment: (await global.nodemodule["axios"]({
url: (await global.nodemodule["axios"]('https://API-VD.miraiprofile2005.repl.co/images/videoanime')).data.data,
method: "GET",
responseType: "stream"
})).data                                      }, threadID, async (error, info) => {
