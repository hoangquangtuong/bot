...
module.exports.config = {
  ...
  version: "1.0.0",
  hasPermssion: 0,
  credits: "Horizon+duy an",
  description: "Ma Sói mod among us by duy an",
  commandCategory: "Game",
  usages: "amongus",
  cooldowns: 1
};
...
module.exports.handleReply = async function({ api,event,handleReply, Users }) {
  ...
  var SoiT = values.player.filter(i => i.phe == "Impostor");
  ...
  if (handleReply.author && event.senderID != handleReply.author) return;
      ...
      case 9: {
        switch (handleReply.type) {
          case "SoiChoose": {
            ...
            for (let i of handleReply.TeamSoi) {
              api.sendMessage('[⚜️]➜ Impostor: ' + name + ' Vừa Vote ' + NanNhan + '\x0aTổng Vote: ' + Global_ArrayChoose['Impostor'][Choose] + '/' + Values['player']['filter'](_0x32101b => _0x32101b['phe'] == 'Impostor')['length'], i);
            }
            api.sendMessage('[⚜️]➜ Bạn Đã Vote: ' + NanNhan + '\x0aTổng Vote: ' + Global_ArrayChoose['Impostor'][Choose] + '/' + Values['player']['filter'](_0x402a9e => _0x402a9e['phe'] == 'Impostor')['length'] + '\x0aBạn Có Thể Thay Đổi Đối Tượng', handleReply.author);
          }
          break;
          case "Another_Role_Async": {
            switch (handleReply.role) {
              case "Angel": {
                ...
                return api.sendMessage('[⚜️]➜ Bạn Đã Chọn Angel: ' + NanNhan + ' Trong Đêm Nay, Bạn Có Thể Chọn Lại !', event.threadID);
              }
              case "guardian": {
                if (isNaN(event.body)) return api.sendMessage("[⚜️]➜ Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
                if (event.body > handleReply.Listuser.length) return api.sendMessage("[⚜️]➜ Sai Rồi, Hãy Nhập Một Con Số !", event.threadID);
        ...
        case "VillageVoting": {
          ...
          api.sendMessage('[⚜️]➜ Vote: ' + name + '(' + event['body'] + ') Thành Công ! \x0aTổng Vote: ' + Global_ArrayChoose['crewmate'][Choose['id']] + '/' + (handleReply['Listuser']['length'] - SoiT['length'] - 0x1), event.threadID);
        }
        break;
      }
    }
...
module.exports.MaSoiChoose = async function(api, event, Users,Data, TeamSoi) {
  ...
  var sus = values.player.filter(i => i.phe == 'Impostor')
  for (let i of sus) {
    var values = Data.get(event.threadID)
    ...
    for (let ii of playerfilter) { 
      ...
      if (ii == playerfilter[playerfilter.length - 1]) {
        ...
        api.sendMessage('Bạn Có 30 Giây Để Lựa Chọn Để Vote Đá đít 1 Người !',i.id)
      }
      stt++;
    }
  }
...
module.exports.Another_Role_Async = async function(api, event, Users,Data) {
  ...
  var playerfilter = values.player;
    for (let i of playerfilter) {
      switch (i.vai) {
        case "Crewmate": {
          api.sendMessage('[⚜️]➜ Không Có Việc Gì Làm, Skip Thôi 🐧',i.id);
        }
        break;
        case "Tiên Tri": {
          let Player = [];
          ...
          api.sendMessage('[⚜️]➜ Hãy Chọn 1 Trong Những Đối Tượng Dưới Đây Để Xem Có Phải Là Impostor Hay Không !', i.id);
        api.sendMessage(Objection,i.id, (err,info) => global.client.handleReply.push({
            type: "Another_Role_Async",
            name: this.config.name,
            author: i.id,
...
module.exports.VillageVoting = async function(api, event, Users,Data,TeamSoi) {
  api.sendMessage('[⚜️]➜ Đã Hết Thời Gian Thảo Luận !, Và Bây Giờ Các Bạn Có 1 Phút Để Vote !', event.threadID);
  ...
  api.sendMessage('[⚜️]➜ Hãy Chọn 1 Trong Những Đối Tượng Dưới Đây Để Vote !', event.threadID);
...
module.exports.VillageLogic_Sort = async function(api, event, Users,Data,TeamSoi) {
  ...
  if (list.length < 1) {
    api.sendMessage('[⚜️]➜ Không Có Ai Bị đá đít Trong Ngày Hôm Nay !', event.threadID);
    return;
  }
    ...
    api.sendMessage('[⚜️]➜ Người Bị đá đít khỏi phi thuyền: ' + name + '\x0aChức Năng: ' + Find['vai'], event.threadID);
    playerfilter.splice(playerfilter.findIndex(i => i.id == index),1);
    if (Find.vai == "Impostor") {
...
module.exports.Morning_Time = async function(api, event, Users,Data,TeamSoi) {
  ...
  try {
    ...
    if (Villager.length <= WereWolf.length) {
      ...
      api.sendMessage("[⚜️]➜ Màn đêm kết thúc, và đây là thông tin của ngày hôm nay !",event.threadID);
      api.sendMessage('Ngày Thứ: ' + Days + '\x0aCòn Sống: ' + All['length'] + '\x0aNgười Chết: ' + (DataGM['Die'] ? DataGM['Die'] : 'Không Có Ai') + '\x0aTổng Sói: ' + Data['get'](event['threadID'])['player']['filter'](_0xb674f8 => _0xb674f8['phe'] == 'Impostor')['length'] + '\x0aTổng Dân: ' + Data['get'](event['threadID'])['player']['filter'](_0x97b918 => _0x97b918['phe'] == 'Crewmate')['length'],event.threadID);
      await new Promise(resolve => setTimeout(resolve, 3000));
      api.sendMessage('[⚜️]➜ Các Bạn Có 1 Phút Để Thảo Luận Đá đít ai ra khỏi phi thuyền !',event.threadID);
...
module.exports.Check_Win_Loop = async function(api, event, Users,Data,TeamSoi) {
  var Values = Data.get(event.threadID);
  var VillageFilter = Values.player.filter(i => i.phe != "Impostor");
  var WolfFilter = Values.player.filter(i => i.phe == "Impostor");
  if (WolfFilter.length == 0) {
    return await module.exports.EndGame(api, event, Data,"Crewmate");
  }
  else if (parseInt(VillageFilter.length) <= parseInt(WolfFilter.length) || parseInt(VillageFilter.length) == 0) {
    return await module.exports.EndGame(api, event, Data,"Impostor");
  }
  else {
...
module.exports.EndGame = async function(api, event,Data,winner) {
  switch (winner) {
    case "Village": {
      api.sendMessage('[⚜️]➜ Chúc Mừng Crewmate Đã Tiêu Diệt Được Tất Cả Impostor ! Và Phần Thắng Thuộc Về Crewmate !!!',event.threadID);
      var values = Data.get(event.threadID);
      var All = values.player;
      var Objection = '';
      for (let i of All) {
    ...
    case "Werewolf": {
      api.sendMessage('[⚜️]➜ Chúc Mừng Impostor Đã Tiêu Diệt Được Tất Cả Crewmate ! Và Phần Thắng Thuộc Về Impostor !!!',event.threadID);
      var values = Data.get(event.threadID);
      var All = values.player;
      var Objection = '';
      for (let i of All) {
  ...
  if (Global_ArrayChoose.MaSoi.hasOwnProperty(Global_ArrayChoose.BaoVe[0])) {
    if (Object.keys(Global_ArrayChoose.MaSoi).length == 2) {
      ...
      if (Global_ArrayChoose.MaSoi.hasOwnProperty(Global_ArrayChoose.BaoVe[0])) {
        ...
        return api.sendMessage('[⚜️]➜ Bạn Đã Bị Impostor Thủ Tiêu Tối Qua !', RandomTarget);
      }
    }
    ...
    if (Object.keys(Global_ArrayChoose.MaSoi).length == 0 || Object.keys(Global_ArrayChoose.MaSoi).length == 1 && Global_ArrayChoose.MaSoi[Object.keys(Global_ArrayChoose.MaSoi)[0]] < WereWolf.length) {
      ...
      if (Object.keys(Global_ArrayChoose.MaSoi).length == 2) {
        ...
        return api.sendMessage('[⚜️]➜ Bạn Đã Bị Impostor Thủ Tiêu Tối Qua !', Object.keys(Global_ArrayChoose.MaSoi)[0]);
      }
    }
  }
  //tiên tri
  ...
  case "CHIA VAI":
    case "CHIAVAI": {
      switch (parseInt(values.type)) {
        case 9: {
          var role = ["Crewmate","Impostor","Angel","Guardian","Impostor","Crewmate","Crewmate","Crewmate","Crewmate"];
            for (let i = 0;i < values.player.length;i++) {
              var vai = role[Math.floor(Math.random() * role.length)];
                var is = dataJson.find(data => data.Name == vai);
                  ...
                  if (vai == "Impostor") Team1.push(values.player[i].id);
          ...
          values.phanvai = 1;global.moduleData.werewolf.set(threadID, values);
            api.sendMessage("[⚜️]➜ Đã Phân Vai Thành Công !, Hãy Kiểm Tra Tin Nhắn Riêng Của Bot, Lưu Ý Nếu Acc Dưới 18 Tuổi Không Được Tham Gia !",event.threadID, event.messageID);
              if (LOI.length != 0) { 
                ...
                return api.sendMessage("[⚜️]➜ Không gửi Được Tin Nhắn Đồng Nghĩa Player Đã Chặn Bot Hoặc Acc Dưới 18+,Game Sẽ Không Thực Hiện Được, Tiến Hành Restart Bot, Hủy Game Tránh Lỗi ( bấm amongus help để biết chi tiết )",threadID, (err, info) => { return process.exit(1); });
              }
            if (Team1.length != 0) {
              try {
      ...
      case "TEST": {
        ...
        if (test.length != 0) {
          for (let kan of test) {
            ...
            api.sendMessage("[⚜️]➜ Phát Hiện Acc Dưới 18+ Hoặc Đã Block Acc Bot" + " Tại User: " + name + ", Tiến Hành Restart Bot, Hủy Game Tránh Lỗi,Chi Tiết Tại amongus help",event.threadID);
          }
  ...
  var out = (msg) => api.sendMessage(msg,event.threadID,event.messageID);var { senderID,threadID,messageID } = event;
    switch (args[0]) {
            ...
            case "Create": {
              if (!global.moduleData.werewolf) global.moduleData.werewolf = new Map();
                var values = global.moduleData.werewolf.get(event.threadID) || {};
                  if (global.moduleData.werewolf.has(event.threadID)) return api.sendMessage("[⚜️]➜ Nhóm đang có Phòng Among Us !", threadID, messageID);
                  global.moduleData.werewolf.set(event.threadID, { author: event.senderID, start: 0, type: '9', phanvai: 0, player: [] });
                global.AuthThread = event.threadID;
              return out("[⚜️]➜ Đã Tạo Thành Công Bàn Có 8-9 Người! Hãy bấm amongus join để tham gia !, Hướng Dẫn Tại amongus help!, Lưu Ý Không Được Cho Acc Dưới 18 Tuổi ( Facebook ) Chơi Vì Sẽ Lỗi !"); 
            }
          case "check":
            case "Check": {
              var values = global.moduleData.werewolf.get(event.threadID) || {};
        ...
        case "Join": {
          var values = global.moduleData.werewolf.get(event.threadID) || {};if (!values.player) return out('Tạo Phòng Đê') ;if (values.player.length >= values.type) return out("Phòng Đã Đầy !");
            if (values.player.find(item => item.id == senderID)) return api.sendMessage("[⚜️]➜ Bạn Đã Tham Gia Rồi !", event.threadID, event.messageID);
              if (!values) return api.sendMessage("[⚜️]➜ Hiện Tại Chưa Có Phòng Among Us Nào Được Mở!", event.threadID, event.messageID);
                if (values.start == 1) return api.sendMessage("[⚜️]➜ Chin Nhỗi Nhưng Phòng Among Us Của Nhóm Này Đã Start !", threadID, messageID);
      ...
      case "leave":
        case "Leave": {
          var values = global.moduleData.werewolf.get(event.threadID) || {};
            if (typeof values.player == "undefined") return api.sendMessage("[⚜️]➜ Hãy Tạo Phòng Among Us Bằng Lệnh amongus số người tham gia :[8-9] Create !", event.threadID, event.messageID);
              if (!values.player.some(item => item.id == senderID)) return api.sendMessage("[⚜️]➜ Bạn chưa tham gia vào Phòng Among Us trong nhóm này!", event.threadID, event.messageID);
                if (values.start == 1) return api.sendMessage("[⚜️]➜ Chin Nhỗi Nhưng Phòng Among Us Của Nhóm Này Đã Start !", threadID, messageID);
          ...
          api.sendMessage("[⚜️]➜ Bạn Đã Rời Khỏi Phòng Among Us Thành Công !", threadID, messageID);
        global.moduleData.werewolf.set(threadID, values);
      }
        }
          break;
      ...
      case 'start':
        ...
        break;
      case "help":
        case "Help": return out(`[⚜️]=== Hướng Dẫn Chơi ===[⚜️]\n\n[⚜️]→ Các Loại: ${global.config.PREFIX}${this.config.name} [9 / status / leave / join]\n[⚜️]→ Tạo Phòng: ${global.config.PREFIX}${this.config.name} 9 create\n[⚜️]→ Kiểm Tra: ${global.config.PREFIX}${this.config.name} 9 check\n[⚜️]→ Rời Phòng: ${global.config.PREFIX}${this.config.name} leave\n[⚜️]→ Tham Gia: ${global.config.PREFIX}${this.config.name} join\n[⚜️]→ Bắt Đầu: ${global.config.PREFIX}${this.config.name} start\n[⚜️]→ Lưu Ý: Không Được Cho Acc Dưới 18 Tuổi ( Facebook ) Chơi Vì Sẽ Lỗi !`);
