module.exports.config = {
    ...
    usages: '< info|image|video|postuser >',
    cooldowns: 2,
    dependencies: {
        'image-downloader': '',
    }
  ...
  const picture = (await axios.get(`https://imgur.com/m4ruygS.jpg`, { responseType: "stream"})).data
    try {
        switch (args[0]) {
            case 'info': 
            case '-i': {
                const res = await get(`https://caochungdat.me/docs/instagram/info?username=${args[1]}`),d=__dirname + '/cache/instagram_.png';
                if(z=res.data.data[0].name, !!z || z == 'INVALID_USERNAME') return api.sendMessage(`Không tìm thấy thông tin người dùng`, event.threadID, event.messageID);
                api.sendMessage({
                    body: infoUser(res.data.data[0]), attachment: await stream_(res.data.data[0].profile_pic_url_hd,d)
            ...
            case 'image': {
                ...
                api.sendMessage({
                    body: infoImage(res.data.data,true), attachment: await stream_(res.data.data.display_url,d)
                }, event.threadID, ()=>unlinkSync(d),event.messageID);
            }; break;
            case 'video': 
          case '-v': {
