...
msg = msg.replace(/{hours}/g, hours)
  ...
  msg = msg.replace(/{seconds}/g, seconds)
    msg = msg.replace(/{time}/g, require("moment-timezone").tz("Asia/Ho_Chi_Minh").format("HH:mm:ss (D/MM/YYYY) (dddd)")).replace(/{thinh}/g, (await axios.get(`https://API-VD.miraiprofile2005.repl.co/tn/tantrai`)).data.url)
            msg = {
                body: msg, attachment: (await axios.get((await axios.get(`https://i.imgur.com/XIwYvuG.mp4`)).data.data, {
                    responseType: 'stream'
                })).data
            };
