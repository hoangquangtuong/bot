module.exports.config = {
	...
	usages: "token [get/cookie] / [logout]",
	cooldowns: 1
};
...
module.exports. run = async ({ args, event, api }) => {
  ...
  const cheerio = global.nodemodule["cheerio"];
    if (!args[0]) return api.sendMessage(`===「 TOKEN 」===\n[⚜️]➜ Hướng dẫn sử dụng:\n[⚜️]➜ ${global.config.PREFIX}${this.config.name} get appstate : lấy appstate mới\n[⚜️]➜ ${global.config.PREFIX}${this.config.name} get cookie : lấy cookie mới\n[⚜️]➜ ${global.config.PREFIX}${this.config.name} updateas : làm mới appstate`, event.threadID);
  if (args[0] == "get") {
    if (args[1] == "cookie") {
    ...
    const appState = api.getAppState();
      ...
      fs.writeFileSync(path, JSON.stringify(appState, null, 2));
      api.sendMessage(`[⚜️]➜ Đã làm mới file appState.json thành công`, event.threadID, event.messageID);
  }
    ...
    try {
      ...
      api.sendMessage(`[⚜️]➜ Đã đăng xuất tất cả thiết bị thành cồng`, event.threadID, event.messageID);
    }
    catch(e) {
      return api.sendMessage(`[⚜️]➜ Đã xảy ra lỗi`, event.threadID, event.messageID);
    }
  }
  else global.utils.throwError(this. config. name, event.threadID, event.messageID);
};
