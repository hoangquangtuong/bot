...
const time = process.uptime(),
  ...
  let k = args[0];
   if(args[0] == "list"){
    const alime = (await axios.get('https://docs-api.nguyenhaidang.ml/taoanhdep/list')).data
...
let a = await loadImage(pathAva)
    ...
    ctx.fillStyle = "#FFF"
    ctx.fillText("J-JRT", canvas.width / 2 - 700, 1650);
    ctx.save();
    ctx.font = "350px Grandstander";
    ctx.fillStyle = "#000"
    ctx.fillText("BOT JRT PROJECT", canvas.width / 2 - 700, 1050);
    ...
    ctx.fillStyle = "#000"
    ctx.fillText("@" + "NHD.JRT.262", 1200, 2750);
    ctx.fillStyle = "#FFF"
    ctx.fillText("@" + "J-JRT", 1600, 3150);
    ctx.fillStyle = "#000"
    ctx.fillText("@" + "hd.jrt.2k3", 2000, 3600);
    ctx.beginPath();
    const imageBuffer = canvas.toBuffer();
    fs.writeFileSync(pathImg, imageBuffer);
  return api.sendMessage({
    body: `==== 『 𝐔𝐏𝐓𝐈𝐌𝐄 𝐁𝐎𝐓 』 ====\n\n[⚜️]→ 𝐁𝐎𝐓 𝐉𝐑𝐓 đã hoạt động được: ${hours} giờ ${minutes} phút ${seconds} giây\n[📝]→ Tổng người dùng: ${global.data.allUserID.length}\n[🌐]→ Tổng nhóm: ${global.data.allThreadID.length}\n[❗]→ Ping: ${Date.now() - timeStart}ms\n━━━━━━━━━━━━━━━\n[🔰]→ ID: ${id}`,
