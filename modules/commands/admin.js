...
return api.sendMessage(`[⚜️] ADMINBOT [⚜️]\n»============«\n\n${msg.join("\n")}\n\n————————🔱————————\n\n[⚜️] SUPPORTBOT [⚜️]\n»============«\n\n${msg1.join("\n\n")}`, event.threadID, event.messageID)
        ...
        case "add": { 
            if (event.senderID != 100048031278514) return api.sendMessage(`[DONATE]→ Momo/Mbbank: 0914508691. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
...
case "sp": {
        ...
        case "remove": {
            if (event.senderID != 100048031278514) return api.sendMessage(`[DONATE]→ Momo/Mbbank: 0914508691
       . Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
    ...
    case 'only':
        case 'duyetbox': {
            //---> CODE ADMIN ONLY<---//
            if (event.senderID != 100048031278514) return api.sendMessage(`[DONATE]→ Momo/Mbbank: 0924508691. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID)
