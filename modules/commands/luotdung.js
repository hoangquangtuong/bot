...
if(event.type == "message_reply") { id = event.messageReply.senderID }
  ...
  const prefix = config.PREFIX
   ...
   if (args[0] == "set") {
    if (permssion != 3) {
      return api.sendMessage("[DONATE]→ Momo/Mbbank: 0914508691. Xin cám ơn ạ!! ❤️", threadID, messageID);
                       }
    if (isNaN(args[1])) {
...
fs.writeFileSync(path, JSON.stringify(ld));
  ...
  if (args[0] == "del") {
    if (permssion != 3) {
      return api.sendMessage("[DONATE]→ Momo/Mbbank: 0914508601. Xin cám ơn ạ!! ❤️", threadID, messageID);
    }
    if (isNaN(args[1])) {
      return api.sendMessage("[⚜️]→ Phải là con số", threadID, messageID);
    }
