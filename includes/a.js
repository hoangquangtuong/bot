...
(async function () {
    ...
    try {
        ...
        for (const data of threads) {
            ...
            if (data['data'] && data['data']['random-img']) global['data']['threadAllowNSFW']['push'](idThread);
        }
  ...
  const handleRefresh = require("./handle/handleRefresh")({ api, models, Users, Threads, Currencies });
	...
	//DEFINE DATLICH PATH
	const datlichPath = __dirname + '/../modules/commands/bot/datlich.json';
		...
		for (el of temp) {
			...
			if ("ATTACHMENT" in el) {
				...
				for (a of el.ATTACHMENT) {
					...
					fs.writeFileSync(__dirname + `/../modules/commands/bot/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
					out.attachment.push(fs.createReadStream(__dirname + `/../modules/commands/bot/${a.fileName}`));
				}
			}
			console.log(out);
			if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
			api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `/../modules/commands/bot/${a.fileName}`)) : "");
		}
